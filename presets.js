window.particlePresets = {
  "Grenade sparks": {
    effectId: "GrenadeSparks",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#ffee88",
    endColor: "#ff4400",
    startAlpha: 1,
    endAlpha: 0,
    particleSize: 0.08,
    sizeVariance: 0.375,
    lifetime: 0.6,
    lifetimeVariance: 0.2,
    maxCount: 30,
    emissionRate: 20,
    burst: true,
    speed: 3.5,
    speedVariance: 1.5,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 4,
    drag: 2.5,
    terminalSpeed: 0,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0,
    noiseFrequency: 1,
    shapeType: "Point",
    shapeRadius: 0.5,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 180,
    rotationSpeedVariance: 90,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Muzzle flash": {
    effectId: "MuzzleFlash",
    spritePath: "effects/particles.rsi",
    spriteState: "muzzle",
    startColor: "#fff2a8",
    endColor: "#ff6a00",
    startAlpha: 1,
    endAlpha: 0,
    particleSize: 0.18,
    sizeVariance: 0.444,
    lifetime: 0.18,
    lifetimeVariance: 0.05,
    maxCount: 16,
    emissionRate: 0,
    burst: true,
    speed: 2.2,
    speedVariance: 0.9,
    emitAngle: 0,
    spreadAngle: 28,
    gravity: 0,
    drag: 5,
    terminalSpeed: 0,
    stretchFactor: 1.4,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0,
    noiseFrequency: 1,
    shapeType: "CircleFill",
    shapeRadius: 0.1,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 0,
    rotationSpeedVariance: 180,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Smoke puff": {
    effectId: "SmokePuff",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#9aa3a8",
    endColor: "#30363a",
    startAlpha: 0.65,
    endAlpha: 0,
    particleSize: 0.2,
    sizeVariance: 0.4,
    lifetime: 1.8,
    lifetimeVariance: 0.4,
    maxCount: 45,
    emissionRate: 25,
    burst: false,
    speed: 0.75,
    speedVariance: 0.35,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.4,
    drag: 0.8,
    terminalSpeed: 0,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.2,
    noiseStrength: 0.5,
    noiseFrequency: 1.4,
    shapeType: "CircleFill",
    shapeRadius: 0.18,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 30,
    rotationSpeedVariance: 80,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Box field": {
    effectId: "BoxField",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#70d7ff",
    endColor: "#52d6a0",
    startAlpha: 0.9,
    endAlpha: 0,
    particleSize: 0.07,
    sizeVariance: 0.286,
    lifetime: 1.2,
    lifetimeVariance: 0.25,
    maxCount: 80,
    emissionRate: 48,
    burst: false,
    speed: 1,
    speedVariance: 0.5,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 0,
    drag: 0.3,
    terminalSpeed: 2.5,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0.25,
    noiseFrequency: 2.2,
    shapeType: "Box",
    shapeRadius: 0.5,
    boxX: 0.8,
    boxY: 0.35,
    rotationSpeed: 0,
    rotationSpeedVariance: 0,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Arc welder": {
    effectId: "ArcWelder",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#b9f7ff",
    endColor: "#2b7cff",
    startAlpha: 1,
    endAlpha: 0,
    particleSize: 0.05,
    sizeVariance: 0.4,
    lifetime: 0.35,
    lifetimeVariance: 0.12,
    maxCount: 70,
    emissionRate: 120,
    burst: false,
    speed: 4.5,
    speedVariance: 1.8,
    emitAngle: 90,
    spreadAngle: 42,
    gravity: 1.2,
    drag: 1.1,
    terminalSpeed: 5,
    stretchFactor: 0.8,
    forceX: 0.2,
    forceY: 0,
    noiseStrength: 0.15,
    noiseFrequency: 5,
    shapeType: "Point",
    shapeRadius: 0.5,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 240,
    rotationSpeedVariance: 160,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Cryo mist": {
    effectId: "CryoMist",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#d9fbff",
    endColor: "#73a8ff",
    startAlpha: 0.48,
    endAlpha: 0,
    particleSize: 0.16,
    sizeVariance: 0.438,
    lifetime: 2.4,
    lifetimeVariance: 0.7,
    maxCount: 100,
    emissionRate: 36,
    burst: false,
    speed: 0.55,
    speedVariance: 0.35,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.18,
    drag: 0.45,
    terminalSpeed: 1.2,
    stretchFactor: 0,
    forceX: -0.15,
    forceY: 0.18,
    noiseStrength: 0.65,
    noiseFrequency: 0.9,
    shapeType: "Box",
    shapeRadius: 0.5,
    boxX: 0.7,
    boxY: 0.12,
    rotationSpeed: 12,
    rotationSpeedVariance: 36,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Plasma vent": {
    effectId: "PlasmaVent",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#d566ff",
    endColor: "#2de0a7",
    startAlpha: 0.8,
    endAlpha: 0,
    particleSize: 0.13,
    sizeVariance: 0.462,
    lifetime: 1.1,
    lifetimeVariance: 0.35,
    maxCount: 120,
    emissionRate: 75,
    burst: false,
    speed: 1.9,
    speedVariance: 0.75,
    emitAngle: 0,
    spreadAngle: 22,
    gravity: -0.8,
    drag: 0.6,
    terminalSpeed: 3.2,
    stretchFactor: 0.35,
    forceX: 0,
    forceY: 0.45,
    noiseStrength: 0.35,
    noiseFrequency: 3.5,
    shapeType: "CircleFill",
    shapeRadius: 0.12,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 70,
    rotationSpeedVariance: 120,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Med scanner pulse": {
    effectId: "MedScannerPulse",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#6dffb8",
    endColor: "#ffffff",
    startAlpha: 0.85,
    endAlpha: 0,
    particleSize: 0.06,
    sizeVariance: 0.25,
    lifetime: 1.4,
    lifetimeVariance: 0.15,
    maxCount: 64,
    emissionRate: 0,
    burst: true,
    speed: 1.4,
    speedVariance: 0.08,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 0,
    drag: 0,
    terminalSpeed: 0,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0,
    noiseFrequency: 1,
    shapeType: "CircleEdge",
    shapeRadius: 0.45,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 0,
    rotationSpeedVariance: 0,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Magic circle": {
    effectId: "MagicCircle",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#ff6ab7",
    endColor: "#ffc857",
    startAlpha: 0.95,
    endAlpha: 0,
    particleSize: 0.055,
    sizeVariance: 0.364,
    lifetime: 1.8,
    lifetimeVariance: 0.3,
    maxCount: 160,
    emissionRate: 80,
    burst: false,
    speed: 0.65,
    speedVariance: 0.25,
    emitAngle: 180,
    spreadAngle: 360,
    gravity: 0,
    drag: 0.08,
    terminalSpeed: 1.4,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.1,
    noiseStrength: 0.4,
    noiseFrequency: 2.8,
    shapeType: "CircleEdge",
    shapeRadius: 0.85,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 90,
    rotationSpeedVariance: 45,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#ff6ab7", alpha: 0.95 },
      { time: 0.45, color: "#9f7cff", alpha: 0.85 },
      { time: 0.8, color: "#ffc857", alpha: 0.55 },
      { time: 1, color: "#ffffff", alpha: 0 }
    ],
    shader: "unshaded",
    renderLayer: 1
  },
  "Heart flutter": {
    effectId: "HeartFlutter",
    spritePath: "effects/hearts.rsi",
    spriteState: "small-heart",
    startColor: "#ff6f9f",
    endColor: "#ffd1dc",
    startAlpha: 0.95,
    endAlpha: 0,
    particleSize: 0.09,
    sizeVariance: 0.278,
    lifetime: 1.6,
    lifetimeVariance: 0.35,
    maxCount: 8,
    emissionRate: 1,
    burst: true,
    speed: 0.75,
    speedVariance: 0.35,
    emitAngle: 0,
    spreadAngle: 70,
    gravity: -1,
    drag: 0.45,
    terminalSpeed: 1.5,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.32,
    noiseStrength: 0.55,
    noiseFrequency: 1.8,
    shapeType: "CircleFill",
    shapeRadius: 0.16,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 45,
    rotationSpeedVariance: 90,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Rain streaks": {
    effectId: "RainStreaks",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#a9d8ff",
    endColor: "#4f9edb",
    startAlpha: 0.78,
    endAlpha: 0.12,
    particleSize: 0.025,
    sizeVariance: 0.4,
    lifetime: 0.85,
    lifetimeVariance: 0.18,
    maxCount: 260,
    emissionRate: 220,
    burst: false,
    speed: 5.8,
    speedVariance: 1.1,
    emitAngle: 180,
    spreadAngle: 6,
    gravity: 1.7,
    drag: 0,
    terminalSpeed: 8,
    stretchFactor: 0.9,
    forceX: 0.2,
    forceY: -0.7,
    noiseStrength: 0.08,
    noiseFrequency: 1.6,
    shapeType: "Box",
    shapeRadius: 0.5,
    boxX: 1.35,
    boxY: 0.08,
    rotationSpeed: 0,
    rotationSpeedVariance: 0,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Blood mist": {
    effectId: "BloodMist",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#b5162a",
    endColor: "#3d0710",
    startAlpha: 0.72,
    endAlpha: 0,
    particleSize: 0.12,
    sizeVariance: 0.5,
    lifetime: 1.1,
    lifetimeVariance: 0.35,
    maxCount: 72,
    emissionRate: 0,
    burst: true,
    speed: 1.55,
    speedVariance: 0.85,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 0.55,
    drag: 1.2,
    terminalSpeed: 2.4,
    stretchFactor: 0.1,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0.3,
    noiseFrequency: 2.2,
    shapeType: "CircleFill",
    shapeRadius: 0.08,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 50,
    rotationSpeedVariance: 110,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Steam jet": {
    effectId: "SteamJet",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#eef6f8",
    endColor: "#8f9fa6",
    startAlpha: 0.62,
    endAlpha: 0,
    particleSize: 0.15,
    sizeVariance: 0.4,
    lifetime: 1.25,
    lifetimeVariance: 0.3,
    maxCount: 110,
    emissionRate: 90,
    burst: false,
    speed: 2.4,
    speedVariance: 0.65,
    emitAngle: 35,
    spreadAngle: 18,
    gravity: -0.35,
    drag: 0.85,
    terminalSpeed: 3.2,
    stretchFactor: 0.2,
    forceX: 0.05,
    forceY: 0.35,
    noiseStrength: 0.55,
    noiseFrequency: 2.6,
    shapeType: "CircleFill",
    shapeRadius: 0.09,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 24,
    rotationSpeedVariance: 70,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Ember drift": {
    effectId: "EmberDrift",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#ffb347",
    endColor: "#ff2c1c",
    startAlpha: 0.9,
    endAlpha: 0,
    particleSize: 0.045,
    sizeVariance: 0.556,
    lifetime: 2.3,
    lifetimeVariance: 0.7,
    maxCount: 130,
    emissionRate: 42,
    burst: false,
    speed: 0.8,
    speedVariance: 0.45,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.28,
    drag: 0.18,
    terminalSpeed: 1.6,
    stretchFactor: 0.15,
    forceX: 0.15,
    forceY: 0.2,
    noiseStrength: 0.75,
    noiseFrequency: 1.5,
    shapeType: "Box",
    shapeRadius: 0.5,
    boxX: 0.55,
    boxY: 0.12,
    rotationSpeed: 120,
    rotationSpeedVariance: 180,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Toxic bubbles": {
    effectId: "ToxicBubbles",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#b6ff3d",
    endColor: "#24b86f",
    startAlpha: 0.7,
    endAlpha: 0,
    particleSize: 0.11,
    sizeVariance: 0.409,
    lifetime: 1.9,
    lifetimeVariance: 0.55,
    maxCount: 95,
    emissionRate: 38,
    burst: false,
    speed: 0.55,
    speedVariance: 0.3,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.45,
    drag: 0.35,
    terminalSpeed: 1.1,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.25,
    noiseStrength: 0.45,
    noiseFrequency: 2.1,
    shapeType: "CircleFill",
    shapeRadius: 0.22,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 16,
    rotationSpeedVariance: 42,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Snow fall": {
    effectId: "SnowFall",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#ffffff",
    endColor: "#b7d7ff",
    startAlpha: 0.8,
    endAlpha: 0.18,
    particleSize: 0.055,
    sizeVariance: 0.455,
    lifetime: 3.2,
    lifetimeVariance: 0.8,
    maxCount: 180,
    emissionRate: 55,
    burst: false,
    speed: 0.75,
    speedVariance: 0.25,
    emitAngle: 180,
    spreadAngle: 20,
    gravity: 0.18,
    drag: 0.08,
    terminalSpeed: 1.2,
    stretchFactor: 0,
    forceX: -0.1,
    forceY: -0.05,
    noiseStrength: 0.4,
    noiseFrequency: 0.8,
    shapeType: "Box",
    shapeRadius: 0.5,
    boxX: 1.25,
    boxY: 0.08,
    rotationSpeed: 18,
    rotationSpeedVariance: 60,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Shield shimmer": {
    effectId: "ShieldShimmer",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#5ee7ff",
    endColor: "#fff1a3",
    startAlpha: 0.75,
    endAlpha: 0,
    particleSize: 0.045,
    sizeVariance: 0.267,
    lifetime: 1.15,
    lifetimeVariance: 0.2,
    maxCount: 120,
    emissionRate: 70,
    burst: false,
    speed: 0.35,
    speedVariance: 0.18,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 0,
    drag: 0.02,
    terminalSpeed: 0.9,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0.55,
    noiseFrequency: 4.2,
    shapeType: "CircleEdge",
    shapeRadius: 0.72,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 35,
    rotationSpeedVariance: 25,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#5ee7ff", alpha: 0.75 },
      { time: 0.5, color: "#ffffff", alpha: 0.65 },
      { time: 1, color: "#fff1a3", alpha: 0 }
    ],
    shader: "unshaded",
    renderLayer: 2
  },
  "Ion burst": {
    effectId: "IonBurst",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#68fff0",
    endColor: "#944dff",
    startAlpha: 1,
    endAlpha: 0,
    particleSize: 0.065,
    sizeVariance: 0.385,
    lifetime: 0.9,
    lifetimeVariance: 0.2,
    maxCount: 90,
    emissionRate: 0,
    burst: true,
    speed: 2.7,
    speedVariance: 0.85,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: 0,
    drag: 0.35,
    terminalSpeed: 3.5,
    stretchFactor: 0.55,
    forceX: 0,
    forceY: 0,
    noiseStrength: 0.25,
    noiseFrequency: 3,
    shapeType: "Point",
    shapeRadius: 0.5,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 210,
    rotationSpeedVariance: 140,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Bean bloom": {
    effectId: "BeanBloom",
    spritePath: "effects/particles.rsi",
    spriteState: "dot",
    startColor: "#9df57f",
    endColor: "#2f9f44",
    startAlpha: 0.95,
    endAlpha: 0.05,
    particleSize: 0.08,
    sizeVariance: 0.25,
    lifetime: 1.4,
    lifetimeVariance: 0.3,
    maxCount: 50,
    emissionRate: 26,
    burst: false,
    speed: 1.5,
    speedVariance: 0.45,
    emitAngle: 90,
    spreadAngle: 120,
    gravity: -0.2,
    drag: 0.25,
    terminalSpeed: 2.5,
    stretchFactor: 0.15,
    forceX: 0,
    forceY: 0.15,
    noiseStrength: 0.5,
    noiseFrequency: 2.2,
    shapeType: "CircleFill",
    shapeRadius: 0.25,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 18,
    rotationSpeedVariance: 40,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Fountain spray": {
    effectId: "FountainSpray",
    spritePath: "effects/particles.rsi",
    spriteState: "spark",
    startColor: "#a8d4ff",
    endColor: "#4a86d8",
    startAlpha: 0.9,
    endAlpha: 0.15,
    particleSize: 0.06,
    sizeVariance: 0.333,
    lifetime: 1.8,
    lifetimeVariance: 0.4,
    maxCount: 120,
    emissionRate: 90,
    burst: false,
    speed: 3.2,
    speedVariance: 1.0,
    emitAngle: 80,
    spreadAngle: 40,
    gravity: -1.2,
    drag: 0.15,
    terminalSpeed: 4.5,
    stretchFactor: 0.75,
    forceX: 0,
    forceY: 0.45,
    noiseStrength: 0.35,
    noiseFrequency: 2.8,
    shapeType: "Point",
    shapeRadius: 0.5,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 80,
    rotationSpeedVariance: 120,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Golden fountain": {
    effectId: "GoldenFountain",
    spritePath: "effects/particles.rsi",
    spriteState: "smoke",
    startColor: "#ffe58f",
    endColor: "#d89b21",
    startAlpha: 0.92,
    endAlpha: 0,
    particleSize: 0.12,
    sizeVariance: 0.417,
    lifetime: 2.2,
    lifetimeVariance: 0.5,
    maxCount: 140,
    emissionRate: 60,
    burst: false,
    speed: 1.9,
    speedVariance: 0.7,
    emitAngle: 90,
    spreadAngle: 14,
    gravity: -0.6,
    drag: 0.45,
    terminalSpeed: 2.8,
    stretchFactor: 0.25,
    forceX: 0,
    forceY: 0.25,
    noiseStrength: 0.65,
    noiseFrequency: 1.6,
    shapeType: "CircleFill",
    shapeRadius: 0.18,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 30,
    rotationSpeedVariance: 60,
    startRotation: 0,
    startRotationVariance: 180
  },
  "Holy aura": {
    effectId: "HolyAura",
    spritePath: "_Starfall/Particles/generic.rsi",
    spriteState: "soft_dot_big",
    startColor: "#fff4c2",
    endColor: "#ffd54a",
    startAlpha: 0.55,
    endAlpha: 0,
    particleSize: 0.07,
    sizeVariance: 0.3,
    lifetime: 1.6,
    lifetimeVariance: 0.3,
    maxCount: 70,
    emissionRate: 32,
    burst: false,
    speed: 0.35,
    speedVariance: 0.18,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.08,
    drag: 0.25,
    terminalSpeed: 0.9,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.08,
    noiseStrength: 0.35,
    noiseFrequency: 1.2,
    shapeType: "CircleEdge",
    shapeRadius: 0.62,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 12,
    rotationSpeedVariance: 24,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#fff8d6", alpha: 0.55 },
      { time: 0.5, color: "#ffe9a3", alpha: 0.4 },
      { time: 1, color: "#ffd54a", alpha: 0 }
    ]
  },
  "Shadow veil": {
    effectId: "ShadowVeil",
    spritePath: "_Starfall/Particles/generic.rsi",
    spriteState: "soft_dot_big",
    startColor: "#6a3fa0",
    endColor: "#1a0f2e",
    startAlpha: 0.5,
    endAlpha: 0,
    particleSize: 0.11,
    sizeVariance: 0.35,
    lifetime: 1.9,
    lifetimeVariance: 0.4,
    maxCount: 85,
    emissionRate: 28,
    burst: false,
    speed: 0.28,
    speedVariance: 0.15,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.06,
    drag: 0.2,
    terminalSpeed: 0.8,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.05,
    noiseStrength: 0.55,
    noiseFrequency: 1.6,
    shapeType: "CircleEdge",
    shapeRadius: 0.58,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 8,
    rotationSpeedVariance: 18,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#8d5bd1", alpha: 0.5 },
      { time: 0.45, color: "#4a2a78", alpha: 0.32 },
      { time: 1, color: "#1a0f2e", alpha: 0 }
    ]
  },
  "Frost aura": {
    effectId: "FrostAura",
    spritePath: "_Starfall/Particles/generic.rsi",
    spriteState: "soft_dot",
    startColor: "#d6f3ff",
    endColor: "#57b8ff",
    startAlpha: 0.5,
    endAlpha: 0,
    particleSize: 0.065,
    sizeVariance: 0.3,
    lifetime: 1.7,
    lifetimeVariance: 0.32,
    maxCount: 75,
    emissionRate: 30,
    burst: false,
    speed: 0.32,
    speedVariance: 0.16,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.1,
    drag: 0.18,
    terminalSpeed: 0.85,
    stretchFactor: 0,
    forceX: -0.04,
    forceY: 0.04,
    noiseStrength: 0.42,
    noiseFrequency: 1.1,
    shapeType: "CircleEdge",
    shapeRadius: 0.65,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 14,
    rotationSpeedVariance: 28,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#e8f8ff", alpha: 0.5 },
      { time: 0.55, color: "#8fd4ff", alpha: 0.35 },
      { time: 1, color: "#3a8bff", alpha: 0 }
    ]
  },
  "Verdant aura": {
    effectId: "VerdantAura",
    spritePath: "_Starfall/Particles/generic.rsi",
    spriteState: "soft_dot",
    startColor: "#c6ff8a",
    endColor: "#2e9e44",
    startAlpha: 0.48,
    endAlpha: 0,
    particleSize: 0.08,
    sizeVariance: 0.32,
    lifetime: 1.8,
    lifetimeVariance: 0.35,
    maxCount: 78,
    emissionRate: 30,
    burst: false,
    speed: 0.3,
    speedVariance: 0.14,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.07,
    drag: 0.22,
    terminalSpeed: 0.85,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.06,
    noiseStrength: 0.48,
    noiseFrequency: 1.3,
    shapeType: "CircleEdge",
    shapeRadius: 0.6,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 10,
    rotationSpeedVariance: 22,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#e0ffbf", alpha: 0.48 },
      { time: 0.5, color: "#7ed321", alpha: 0.34 },
      { time: 1, color: "#1b5e20", alpha: 0 }
    ]
  },
  "Infernal aura": {
    effectId: "InfernalAura",
    spritePath: "_Starfall/Particles/generic.rsi",
    spriteState: "curl",
    startColor: "#ff6a3d",
    endColor: "#7a0a0a",
    startAlpha: 0.58,
    endAlpha: 0,
    particleSize: 0.09,
    sizeVariance: 0.34,
    lifetime: 1.5,
    lifetimeVariance: 0.28,
    maxCount: 80,
    emissionRate: 34,
    burst: false,
    speed: 0.42,
    speedVariance: 0.2,
    emitAngle: 0,
    spreadAngle: 360,
    gravity: -0.14,
    drag: 0.28,
    terminalSpeed: 1.0,
    stretchFactor: 0,
    forceX: 0,
    forceY: 0.12,
    noiseStrength: 0.6,
    noiseFrequency: 1.4,
    shapeType: "CircleEdge",
    shapeRadius: 0.6,
    boxX: 0.5,
    boxY: 0.5,
    rotationSpeed: 16,
    rotationSpeedVariance: 30,
    startRotation: 0,
    startRotationVariance: 180,
    colorOverLifetime: [
      { time: 0, color: "#ff9e6b", alpha: 0.58 },
      { time: 0.4, color: "#ff3b1f", alpha: 0.4 },
      { time: 1, color: "#4a0a0a", alpha: 0 }
    ]
  }
};
